...
Процедура ПередЗакрытиеБезРежимаСовместимости(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если Модифицированность Тогда
		...
		Иначе
			ТекстПредупреждения = НСтр("ru = 'Данные шага ""Выполнить вручную"" были изменены. При закрытии все изменения будут утеряны'");
		КонецЕсли;
...
Функция СцТ_ПрименитьИзменения_НаКлиенте() Экспорт
	...
	Если ПустаяСтрока(ДШ_Описание) Тогда
		ТекстСообщения = НСтр("ru = 'Необходимо написать содержание ручной операции'");
...
Функция СцТ_СформироватьОписаниеШага_НаКлиенте(Краткое = Ложь)
	...
	ВозвращаемоеОписание = НСтр("ru = 'Ручная операция'");
