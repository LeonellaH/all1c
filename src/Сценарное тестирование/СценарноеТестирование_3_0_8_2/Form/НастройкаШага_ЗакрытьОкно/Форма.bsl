...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОА_ТипШага = "ЗавершитьРаботуТестируемогоКлиента" Тогда
		Элементы.Страница_ДанныеШага.Видимость = Истина;
		
		РазорватьСвязьСТестируемымПриложением = СцТ_ЗначениеВДереве(ДеревоЗначения, "РазорватьСвязьСТестируемымПриложением", Ложь);
		
	Иначе
		Элементы.Страница_ДанныеШага.Видимость = Ложь;
...
Процедура ПередЗакрытиеБезРежимаСовместимости(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если Модифицированность Тогда
		...
		Иначе
			ТекстПредупреждения = НСтр("ru = 'Данные шага ""Закрытие окна"" были изменены. При закрытии все изменения будут утеряны'");
		КонецЕсли;
...
Процедура СцТ_ПодготовитьДанныеШагаДляСохранения_НаКлиенте(Отказ)
	...
	Если ОА_ТипШага = "ЗавершитьРаботуТестируемогоКлиента" Тогда
		СцТ_ГлавнаяФорма.СцТ_ДобавитьУзелВДеревоЗначения(
			ДеревоЗначения,
			"РазорватьСвязьСТестируемымПриложением",
			РазорватьСвязьСТестируемымПриложением);
...
Функция СцТ_СформироватьОписаниеШага_НаКлиенте(Краткое = Ложь)
	...
	Если ОА_ТипШага = "ЗакрытьКлиентскоеОкноФормы" Тогда
		Если Краткое Тогда
			Возврат НСтр("ru = 'Закрыть окно текущей формы'");
		Иначе
			Возврат НСтр("ru = 'Закрыть окно, в котором располагается текущая форма'");
		КонецЕсли;
	Иначе
		Если Краткое Тогда
			Возврат НСтр("ru = 'Закрыть тестируемое приложение'");
		Иначе
			Возврат НСтр("ru = 'Закрыть главное окно тестируемого приложения'");
		КонецЕсли;
	КонецЕсли;
