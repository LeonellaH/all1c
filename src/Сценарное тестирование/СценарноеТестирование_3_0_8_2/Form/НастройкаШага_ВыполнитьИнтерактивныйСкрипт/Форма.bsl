...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.ОткрытаПодВнешнимУправлением Тогда
		...
		Элементы.РаботаСЖурналомДействийПользователя.Доступность = Ложь;
...
Процедура ПриОткрытии(Отказ)
	...
	ТекущийЭлемент = Элементы.ТекстСкрипта;
...
Процедура ПередЗакрытиеБезРежимаСовместимости(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если Модифицированность Тогда
		...
		Иначе
			ТекстПредупреждения = НСтр("ru = 'Данные группового шага ""Воспроизвести журнал действий пользователя"" были изменены. При закрытии все изменения будут утеряны'");
		КонецЕсли;
...
Процедура СцТ_НастроитьФормуДляШага()
	...
	ТекстСкрипта = СцТ_ЗначениеВДереве(ДеревоЗначения, "ТекстСкрипта", "");
...
Функция СцТ_ПрименитьИзменения_НаКлиенте() Экспорт
	...
	Если ПустаяСтрока(ТекстСкрипта) Тогда
		ТекстСообщения = НСтр("ru = 'Текст скрипта должен быть указан'");
		
		СП = Новый СообщениеПользователю;
		СП.Текст = ТекстСообщения;
		СП.ПутьКДанным = "ТекстСкрипта";
		СП.Сообщить();
...
Функция СцТ_СформироватьОписаниеШага_НаКлиенте(Краткое = Ложь)
	...
	ВозвращаемоеОписание = НСтр("ru = 'Выполнить действия из журнала действий пользователя'");
...
&НаКлиенте
Процедура РаботаСЖурналомДействийПользователя(Команда)
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ЗакрыватьПриЗакрытииВладельца", Истина);
	ПараметрыФормы.Вставить("РежимОткрытияФормыЖурнала"    , "ЖурналДействий");
	ПараметрыФормы.Вставить("ТекстЖурнала"                 , ТекстСкрипта);
	...
	ФормаЗаписиЖурнала = ОткрытьФорму(ПутьКФормам + "РаботаСЖурналомДействийПользователя",
		ПараметрыФормы,
		ЭтотОбъект,,,,
		Новый ОписаниеОповещения("СцТ_ПриОкончанииЗаписиЖурналаИД", ЭтотОбъект));
...
//
&НаКлиенте
Процедура СцТ_ПриОкончанииЗаписиЖурналаИД(ЗаписанныйЖурнал, ДопПараметры) Экспорт
	
	Если ЗаписанныйЖурнал <> Неопределено Тогда
		ТекстСкрипта = ЗаписанныйЖурнал;
	КонецЕсли;
