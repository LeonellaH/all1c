...
Процедура ПередЗакрытиеБезРежимаСовместимости(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если Модифицированность Тогда
		...
		Иначе
			ТекстПредупреждения = НСтр("ru = 'Данные группового шага ""Выполнить запрос"" были изменены. При закрытии все изменения будут утеряны'");
		КонецЕсли;
...
Функция СцТ_СформироватьОписаниеШага_НаКлиенте(Краткое = Ложь)
	...
	ВозвращаемоеОписание = СцТ_ГлавнаяФорма.СцТ_ПолучитьОписаниеШага_ВыполнитьЗапрос();
...
Процедура СцТ_ВыполнитьЗапрос(СтруктураГлобальныхПараметров)
	...
	Если ПолученныйРезультатЗапроса.Количество() > 100 Тогда
		ТекстСообщения = НСтр("ru = 'Результат запроса содержит более 100 строк. Измените условие запроса или добавтье в текст запроса ограничение ""ПЕРВЫЕ 100""'");
