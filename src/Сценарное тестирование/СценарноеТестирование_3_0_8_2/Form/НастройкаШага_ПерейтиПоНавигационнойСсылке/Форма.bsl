...
Процедура ПриОткрытии(Отказ)
	...
	ТекущийЭлемент = Элементы.СцТ_НавигационнаяСсылка;
...
Процедура ПередЗакрытиеБезРежимаСовместимости(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если Модифицированность Тогда
		...
		Иначе
			ТекстПредупреждения = НСтр("ru = 'Данные шага ""Переход по навигационной ссылке"" были изменены. При закрытии все изменения будут утеряны'");
		КонецЕсли;
...
Процедура СцТ_НастроитьФормуДляШага()
	...
	СцТ_НавигационнаяСсылка = СцТ_ЗначениеВДереве(ДеревоЗначения, "НавигационнаяСсылка", "");
...
Функция СцТ_ПрименитьИзменения_НаКлиенте() Экспорт
	...
	Если ПустаяСтрока(СцТ_НавигационнаяСсылка) Тогда
		ТекстСообщения = НСтр("ru = 'Навигационная ссылка должна быть указана'");
		
		СП = Новый СообщениеПользователю;
		СП.Текст = ТекстСообщения;
		СП.ПутьКДанным = "СцТ_НавигационнаяСсылка";
		СП.Сообщить();
...
Функция СцТ_СформироватьОписаниеШага_НаКлиенте(Краткое = Ложь)
	...
	ВозвращаемоеОписание = НСтр("ru = 'Перейти по навигационной ссылке'");
