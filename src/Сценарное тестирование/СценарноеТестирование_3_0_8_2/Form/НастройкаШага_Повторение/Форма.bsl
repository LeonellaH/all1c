...
Процедура ПриОткрытии(Отказ)
	...
	ТекущийЭлемент = Элементы.ПовторениеТипЦикла;
...
Процедура ПередЗакрытиеБезРежимаСовместимости(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если Модифицированность Тогда
		...
		Иначе
			ТекстПредупреждения = НСтр("ru = 'Данные группового шага ""Повторение"" были изменены. При закрытии все изменения будут утеряны'");
		КонецЕсли;
...
Процедура СцТ_НастроитьФормуДляШага()
	...
	ПовторениеТипЦикла           = "Счетный";
	ПовторениеИмяПеременной      = "ПеременнаяЦикла";
	ПовторениеКонечноеЗначение   = 1;
	ПовторениеНачальноеЗначение  = 1;
	ПовторениеШаг                = 1;
	ПовторениеКоличествоПовторов = 1;
	
	УзелОписаниПовтора = СцТ_НайтиУзелДереваФормы(ДеревоЗначения, "Имя", "ОписаниеПовтора");
	Если УзелОписаниПовтора <> Неопределено Тогда
		ПовторениеТипЦикла           = СцТ_ЗначениеВДереве(УзелОписаниПовтора, "ТипЦикла"          , "Счетный");
		ПовторениеИмяПеременной      = СцТ_ЗначениеВДереве(УзелОписаниПовтора, "ИмяПеременной"     , "ПеременнаяЦикла");
		ПовторениеКонечноеЗначение   = СцТ_ЗначениеВДереве(УзелОписаниПовтора, "КонечноеЗначение"  , 1);
		ПовторениеНачальноеЗначение  = СцТ_ЗначениеВДереве(УзелОписаниПовтора, "НачальноеЗначение" , 1);
		ПовторениеШаг                = СцТ_ЗначениеВДереве(УзелОписаниПовтора, "Шаг"               , 1);
		ПовторениеКоличествоПовторов = СцТ_ЗначениеВДереве(УзелОписаниПовтора, "КоличествоПовторов", 1);
	КонецЕсли;
	
	
	
	УправлениеДоступностьюСтраницПовторения(ПовторениеТипЦикла, ЭтотОбъект);
	СформироватьТекстыЦиклов(
		ПовторениеИмяПеременной,
		ПовторениеНачальноеЗначение,
		ПовторениеКонечноеЗначение,
		ПовторениеШаг,
		Элементы.НадписьПовторениеСчетныйЦикл.Заголовок,
		Элементы.НадписьПовторениеЦиклПока.Заголовок);
...
Процедура СцТ_ПодготовитьДанныеШагаДляСохранения_НаКлиенте(Отказ)
	...
	Если ПустаяСтрока(ПовторениеИмяПеременной) Тогда
		СцТ_СообщитьПользователю(
			НСтр("ru = 'Поле ""Имя переменной"" не заполнено'"),
			Отказ);
	КонецЕсли;
	
	Если ПустаяСтрока(ПовторениеТипЦикла) Тогда
		СцТ_СообщитьПользователю(
			НСтр("ru = 'Поле ""Тип цикла"" не заполнено'"),
			Отказ,
			"ПовторениеТипЦикла");
	КонецЕсли;
	...
	УзелОписанияЦикла = СцТ_ГлавнаяФорма.СцТ_ДобавитьУзелВДеревоЗначения(
		ДеревоЗначения,
		"ОписаниеПовтора",
		0,
	...
	СцТ_ГлавнаяФорма.СцТ_ДобавитьУзелВДеревоЗначения(
		...
		"КонечноеЗначение",
		ПовторениеКонечноеЗначение,
		НСтр("ru = 'Конечное значение'"),
		Истина);
		
	СцТ_ГлавнаяФорма.СцТ_ДобавитьУзелВДеревоЗначения(
		УзелОписанияЦикла,
		"КоличествоПовторов",
		ПовторениеКоличествоПовторов,
		НСтр("ru = 'Количество повторов'"),
		Истина);
...
Функция СцТ_СформироватьОписаниеШага_НаКлиенте(Краткое = Ложь)
	...
	Если ПовторениеТипЦикла = "Счетный" Тогда
		
		СтрокаКоличества = Строка(ПовторениеКоличествоПовторов);
		Если Найти("0156789", Прав(СтрокаКоличества, 1)) Тогда
			ВозвращаемоеОписание = НСтр("ru = 'Повторить %1 раз'");
		Иначе
			ВозвращаемоеОписание = НСтр("ru = 'Повторить %1 раза'");
		КонецЕсли;
		
		ВозвращаемоеОписание = СтрЗаменить(ВозвращаемоеОписание, "%1", СтрокаКоличества);
		
	Иначе
		ВозвращаемоеОписание = НСтр("ru = 'Повторять пока значение переменной %1 Истина'");
		ВозвращаемоеОписание = СтрЗаменить(ВозвращаемоеОписание, "%1", ПовторениеИмяПеременной);
...
&НаКлиенте
Процедура ПовторениеТипЦиклаПриИзменении(Элемент)
...
&НаКлиентеНаСервереБезКонтекста
Процедура УправлениеДоступностьюСтраницПовторения(ТипЦикла, Форма)
	
	Если ТипЦикла = "Счетный" Тогда
		Форма.Элементы.СтраницыПовторения.ТекущаяСтраница = Форма.Элементы.СтраницаПовторениеСчетныйЦикл;
		
	Иначе
		Форма.Элементы.СтраницыПовторения.ТекущаяСтраница = Форма.Элементы.СтраницаПовторенияЦиклПока;
...
&НаКлиенте
Процедура ПовторениеИмяПеременной1ПриИзменении(Элемент)
	
	Если НЕ СцТ_ГлавнаяФорма.ПроверитьПравильностьИмениПараметра(ПовторениеИмяПеременной) Тогда
		ПовторениеИмяПеременной = НСтр("ru = 'ПеременнаяЦикла'");
	КонецЕсли;
...
&НаКлиентеНаСервереБезКонтекста
Процедура СформироватьТекстыЦиклов(ИмяПеременной, Начало, Конец, Шаг, ЗаголовокСчетного, ЗаголовокПока)
	
	ЗаголовокСчетного = НСтр("ru = 'Для %1 = %2 По %3 с шагом %4 Цикл'");
	ЗаголовокПока     = НСтр("ru = 'Пока %1 Цикл'");
	...
	Иначе
		ЗаголовокСчетного = НСтр("ru = 'Для %1 = %2 По %3 с шагом %4 Цикл'");
		ЗаголовокПока     = НСтр("ru = 'Пока %1 Цикл'");
		ЗаголовокСчетного = СтрЗаменить(ЗаголовокСчетного, "%1", ИмяПеременной);
		ЗаголовокПока     = СтрЗаменить(ЗаголовокПока, "%1", ИмяПеременной);
		
		ЗаголовокСчетного = СтрЗаменить(ЗаголовокСчетного, "%2", Строка(Начало));
		ЗаголовокСчетного = СтрЗаменить(ЗаголовокСчетного, "%3", Строка(Конец));
		ЗаголовокСчетного = СтрЗаменить(ЗаголовокСчетного, "%4", Строка(Шаг));
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ПовторениеКоличествоПовторовПриИзменении(Элемент)
	
	РассчитатьКонечноеЗначениеИСформироватьТекст();
	
КонецПроцедуры

&НаКлиенте
Процедура РассчитатьКонечноеЗначениеИСформироватьТекст()
	
	ПовторениеКонечноеЗначение = ПовторениеНачальноеЗначение + ПовторениеШаг * (ПовторениеКоличествоПовторов - 1);
...
&НаКлиенте
Процедура ПовторениеИмяПеременнойПриИзменении(Элемент)
...
&НаКлиенте
Процедура ПовторениеНачальноеЗначениеПриИзменении(Элемент)
...
&НаКлиенте
Процедура ПовторениеШагПриИзменении(Элемент)
