...
Процедура ПередЗакрытиеБезРежимаСовместимости(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если Модифицированность Тогда
		...
		Иначе
			ТекстПредупреждения = НСтр("ru = 'Данные шага ""Проверить движения документа"" были изменены. При закрытии все изменения будут утеряны'");
		КонецЕсли;
...
Процедура СравнитьТаблицуВизуально(Команда)
	...
	Если ТекущаяСтраницаСтруктурыДанных = Неопределено Тогда
		Возврат;
	КонецЕсли;
...
Процедура ДанныеТаблицыВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Для каждого УзелДерева Из ДеревоЗначения.ПолучитьЭлементы() Цикл
		Если УзелДерева.Имя = "ДвиженияПоРегистрам" Тогда
			УзелТЧ = УзелДерева;
			Прервать;
		КонецЕсли;
	КонецЦикла;
...
Процедура СцТ_НастроитьФормуДляШага()
	...
	СцТ_ОтобразитьДвижения(ДеревоЗначенияНаСервере);
...
//
&НаСервере
Процедура СцТ_ОтобразитьДвижения(ДеревоЗначения);
	
	УзелДвижений = ДеревоЗначения.Строки.Найти("ДвиженияПоРегистрам", "Имя", Истина);
	...
	Для каждого УзелДвижения Из УзелДвижений.Строки Цикл
		...
		Если Элементы.Найти("Страница" + УзелДвижения.Имя) = Неопределено Тогда
			// Добавление таблиц
			ЭтоНоваяТаблица = Истина;
			МассивНовыхРеквизитов.Добавить(
				Новый РеквизитФормы(
					"СцТ_Таблица_" + УзелДвижения.Имя,
	...
	Для каждого УзелДвижения Из УзелДвижений.Строки Цикл
		СцТ_ОтобразитьТаблицуНаФорме(УзелДвижения, Элементы.СтраницыСтруктурыОбъекта);
		СцТ_ЗаполнитьТаблицуЗначениями(УзелДвижения);
		Если УзелДвижения.Активность Тогда
			КоличествоАктивных = КоличествоАктивных + 1;
		КонецЕсли;
	КонецЦикла;
	
	Если КоличествоАктивных = 0 Тогда
		Элементы.СтраницыДанныхШага.ТекущаяСтраница = Элементы.СтраницыДанныхШага_Пустая;
	Иначе
		Элементы.СтраницыДанныхШага.ТекущаяСтраница = Элементы.СтраницыДанныхШага_Заполненная;
...
Функция СцТ_СформироватьОписаниеШага_НаКлиенте(Краткое = Ложь)
	...
	Если Краткое Тогда
		...
		Иначе
			ВозвращаемоеОписание = СцТ_ГлавнаяФорма.СцТ_ПолучитьОписаниеШага_СравнитьДвижения(Неопределено, Истина, СцТ_РодительУзла);
		КонецЕсли;
		
	Иначе
		
		ВозвращаемоеОписание = СцТ_СформироватьОписаниеШага_НаСервере();
...
&НаСервере
Функция СцТ_СформироватьОписаниеШага_НаСервере()
	
	МодульОбъекта = РеквизитФормыВЗначение("Объект");
	ДеревоЗначенийШага = РеквизитФормыВЗначение("ДеревоЗначения");
	
	ВозвращаемоеЗначение = МодульОбъекта.СцТ_ПолучитьОписаниеШага_СравнитьДвижения_НаСервере(
		ДеревоЗначенийШага);
...
Процедура СцТ_ОбновитьОтображениеВСоответствииСНастройкой(АдресХранилища)
	...
	СцТ_ОбновитьНастройкиДвижений(ДеревоНастройки, ДеревоЗначенияНаСервере);
	Модифицированность = Истина;
	
	ЗначениеВРеквизитФормы(ДеревоЗначенияНаСервере, "ДеревоЗначения");
	
	КоличествоАктивных = 0;
	Для каждого СтраницаДвижения Из Элементы.СтраницыСтруктурыОбъекта.ПодчиненныеЭлементы Цикл
		Если СтраницаДвижения.Видимость Тогда
			КоличествоАктивных = КоличествоАктивных + 1;
		КонецЕсли;
...
&НаСервере
Процедура СцТ_ОбновитьНастройкиДвижений(ДеревоНастройки, ДеревоДанных)
	
	УзелДвижений_Данные    = ДеревоДанных.Строки.Найти("ДвиженияПоРегистрам", "Имя");
	УзелДвижений_Настройки = ДеревоНастройки;
	
	Если УзелДвижений_Данные = Неопределено ИЛИ УзелДвижений_Настройки = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Для каждого ОписаниеТЧ_Данные Из УзелДвижений_Данные.Строки Цикл
		
		ОписаниеТЧ_Настройки = УзелДвижений_Настройки.Строки.Найти(ОписаниеТЧ_Данные.Имя, "Имя");
		...
		ИндексТаблицы_Данные    = УзелДвижений_Данные.Строки.Индекс(ОписаниеТЧ_Данные);
		ИндексТаблицы_Настройка = УзелДвижений_Настройки.Строки.Индекс(ОписаниеТЧ_Настройки);
		...
		Если ИндексТаблицы_Данные <> ИндексТаблицы_Настройка Тогда
			...
			Если Дельта <> 0 Тогда
				УзелДвижений_Данные.Строки.Сдвинуть(ОписаниеТЧ_Данные, Дельта);
...
Функция СцТ_ПодготовитьДанныеДляВизуальногоСравненияТаблицНаСервере(
	...
	Документ = МодульЭтойОбработки.ПолучитьОбъектИлиЗаписьПоСтруктуреПоиска(
		ПараметрыДляПоискаОбъекта,
		АдресТаблицыСсылок,
		ГлобальныеПараметры);
	...
	УзелТаблиц = ДеревоНаСервере.Строки.Найти("ДвиженияПоРегистрам", "Имя");
	
	Если УзелТаблиц = Неопределено Тогда
		ТекстСообщения = НСтр("ru = 'Не удалось найти узел движений в структуре данных шага.'");
		Возврат Неопределено;
	КонецЕсли;
	...
	Если УзелТаблицы = Неопределено Тогда
		ТекстСообщения = НСтр("ru = 'Не удалось найти узел движения %1 в структуре данных шага.'");
	...
	Если УзелТаблицы.Значение = "РегистрСведений" Тогда
		МенеджерРегистров = РегистрыСведений;
	ИначеЕсли УзелТаблицы.Значение = "РегистрНакопления" Тогда
		МенеджерРегистров = РегистрыНакопления;
	ИначеЕсли УзелТаблицы.Значение = "РегистрБухгалтерии" Тогда
		МенеджерРегистров = РегистрыБухгалтерии;
	ИначеЕсли УзелТаблицы.Значение = "РегистрРасчета" Тогда
		МенеджерРегистров = РегистрыРасчета;
	КонецЕсли;
	
	НаборЗаписей = МенеджерРегистров[УзелТаблицы.Имя].СоздатьНаборЗаписей();
	НаборЗаписей.Отбор.Регистратор.Установить(Документ.Ссылка);
	НаборЗаписей.Прочитать();
	
	ТестоваяТаблица = НаборЗаписей.Выгрузить();
	КолонкаМоментВремени = ТестоваяТаблица.Колонки.Найти("МоментВремени");
	Если КолонкаМоментВремени <> Неопределено Тогда
		ТестоваяТаблица.Колонки.Удалить(КолонкаМоментВремени);
	КонецЕсли;
	
	ТипОбъекта = "ДвиженияДокумента";
...
Процедура СцТ_ПрименитьНастройкиКлючей(ДанныеКлючей)
	...
	УзелТаблиц = СцТ_ГлавнаяФорма.СцТ_НайтиУзелДереваФормы(ДеревоЗначения, "Имя", "ДвиженияПоРегистрам");
