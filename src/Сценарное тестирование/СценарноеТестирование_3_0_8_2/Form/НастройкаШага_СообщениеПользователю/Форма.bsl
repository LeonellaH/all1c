...
Процедура ПриОткрытии(Отказ)
	...
	ТекущийЭлемент = Элементы.ВыбратьСообщениеДействие;
...
Процедура ПередЗакрытиеБезРежимаСовместимости(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если Модифицированность Тогда
		...
		Иначе
			ТекстПредупреждения = НСтр("ru = 'Данные шага ""Сообщение пользователю"" были изменены. При закрытии все изменения будут утеряны'");
		КонецЕсли;
...
Процедура СцТ_НастроитьФормуДляШага()
	...
	УзелВыбранногоСообщения = СцТ_НайтиУзелДереваФормы(ДеревоЗначения, "Имя", "ВыбранноеСообщениеПользователю");
	Если УзелВыбранногоСообщения = Неопределено Тогда
		// Заполнение по умолчанию
		ВыбратьСообщениеСообщение = "*";
		
	Иначе
		// Заполнение из ИБ
		ВыбратьСообщениеСообщение = УзелВыбранногоСообщения.Значение;
	КонецЕсли;
	
	// Заполнение списка действий
	Элементы.ВыбратьСообщениеДействие.СписокВыбора.Очистить();
	Элементы.ВыбратьСообщениеДействие.СписокВыбора.Добавить("Сведения" , НСтр("ru = 'Получить сведения'"));
	Элементы.ВыбратьСообщениеДействие.СписокВыбора.Добавить("Проверить", НСтр("ru = 'Проверить'"));
	
	УзелДействия = СцТ_НайтиУзелДереваФормы(ДеревоЗначения, "Имя", "Действие");
	Если УзелДействия <> Неопределено Тогда
		ВыбратьСообщениеДействие = УзелДействия.Значение;
	Иначе
		ВыбратьСообщениеДействие = "Сведения";
	КонецЕсли;
...
Функция СцТ_ПрименитьИзменения_НаКлиенте() Экспорт
	...
	Если ПустаяСтрока(ВыбратьСообщениеСообщение) Тогда
		
		ТекстСообщения = НСтр("ru = 'Не заполнен текст сообщения. Необходимо либо заполнить текст сообщения, либо воспользоваться другим шагом для проверки отсутствия сообщений'");
		
		СП = Новый СообщениеПользователю;
		СП.Текст = ТекстСообщения;
		СП.ПутьКДанным = "ВыбратьСообщениеСообщение";
		СП.Сообщить();
...
Если ПустаяСтрока(ВыбратьСообщениеДействие) Тогда
		ВыбратьСообщениеДействие = "Сведения";
	КонецЕсли;
...
Функция СцТ_СформироватьОписаниеШага_НаКлиенте(Краткое = Ложь)
	...
	Если ВыбратьСообщениеДействие = "Сведения" Тогда
		
		ВозвращаемоеОписание = НСтр("ru = 'Получить сведения о выборе сообщения пользователю'");
		
	Иначе
		// Проверка
		ВозвращаемоеОписание = НСтр("ru = 'Проверить выбор сообщения пользователю'");
		
	КонецЕсли;
	
	Если НЕ Краткое Тогда
		ВозвращаемоеОписание = ВозвращаемоеОписание + ":" +Символы.ПС + ВыбратьСообщениеСообщение;
	КонецЕсли;
...
Процедура СцТ_ПолучитьИнтерактивныйКонтейнер()
	...
	ИнтерактивныйКонтейнер = СцТ_ГлавнаяФорма.СцТ_ПолучитьТестируемуюФормуДляИнтерактивнойАвторгруппы(
			РодительУзла,,
			ОписаниеОшибкиПолученияКонтейнера,
			Истина);
	...
	Если ТипЗнч(ИнтерактивныйКонтейнер) <> Тип("ТестируемоеОкноКлиентскогоПриложения") Тогда
		ИнтерактивныйКонтейнер = Неопределено;
...
&НаКлиенте
Процедура ВыбратьСообщениеСообщениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	
	Если ФормаОткрытаПодВнешнимУправлением Тогда
		ТекстСообщения = НСтр("ru = 'Операция недоступна'");
		ПоказатьПредупреждение(, ТекстСообщения);
		Возврат;
	КонецЕсли;
	...
	ТестируемоеОкно = ИнтерактивныйКонтейнер;
	
	Если ТестируемоеОкно <> Неопределено Тогда
		МассивСообщений = ТестируемоеОкно.ПолучитьТекстыСообщенийПользователю();
		Если МассивСообщений.Количество() = 0 Тогда
			ТекстПредупреждения = НСтр("ru = 'Окно не содержит сообщений пользователю'");
			ПоказатьПредупреждение(, ТекстПредупреждения);
			Возврат;
		КонецЕсли;
	...
	СписокСообщений.ЗагрузитьЗначения(Новый Массив(МассивСообщений));
	
	ТекущийЭлементСписка = СписокСообщений.НайтиПоЗначению(ВыбратьСообщениеСообщение);
	
	ОповещениеОВыборе = Новый ОписаниеОповещения("ПриВыбореСообщенияПользователю", ЭтотОбъект);
	
	СписокСообщений.ПоказатьВыборЭлемента(ОповещениеОВыборе, НСтр("ru = 'Список текущих сообщений'"), ТекущийЭлементСписка);
	
КонецПроцедуры

&НаКлиенте
Процедура ПриВыбореСообщенияПользователю(Результат, ДопПараметры) Экспорт
	
	Если Результат = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ВыбратьСообщениеСообщение = Результат.Значение;
