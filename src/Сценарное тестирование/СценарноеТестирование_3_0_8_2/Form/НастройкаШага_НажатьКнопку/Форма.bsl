...
Процедура ПередЗакрытиеБезРежимаСовместимости(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если Модифицированность Тогда
		...
		Иначе
			ТекстПредупреждения = НСтр("ru = 'Данные шага ""Нажать кнопку"" были изменены. При закрытии все изменения будут утеряны'");
		КонецЕсли;
...
Процедура СцТ_НастроитьФормуДляШага()
	...
	СВТ.Добавить("КнопкаКоманднойПанели", НСтр("ru = 'Кнопка командной панели'"));
	СВТ.Добавить("ОбычнаяКнопка"        , НСтр("ru = 'Кнопка формы'"));
	СВТ.Добавить("Гиперссылка"          , НСтр("ru = 'Гиперссылка'"));
	СВТ.Добавить("Картинка"             , НСтр("ru = 'Картинка'"));
	СВТ.Добавить("Надпись"              , НСтр("ru = 'Надпись'"));
	СВТ.Добавить("ПолеФлажка"           , НСтр("ru = 'Флажок'"));
	СВТ.Добавить("Декорация"            , НСтр("ru = 'Декорация'"));
	СВТ.Добавить("Поле"                 , НСтр("ru = 'Поле формы'"));
	
	ЭлементФормы_Вид = СцТ_ЗначениеВДереве(ДеревоЗначения, "ВидОбъекта", "КнопкаКоманднойПанели");
	
	УзелОписанияЭлемента = СцТ_НайтиУзелДереваФормы(ДеревоЗначения, "Имя", "ИмяКнопки");
	...
	ДействиеСКнопкой = СцТ_ЗначениеВДереве(ДеревоЗначения, "ДействиеСКнопкой", "Нажать");
	
	ЗаполнитьПутьКЭлементуФормы(ДеревоЗначения);
	
	ЗначениеВариантаПоУмолчанию = "100";
	Если ПустаяСтрока(ЭлементФормы_Имя) ИЛИ Лев(ЭлементФормы_Имя, 1) = "$"Тогда
		ЗначениеВариантаПоУмолчанию = "011";
	КонецЕсли;
	
	ВариантПоискаОбъекта = СцТ_ЗначениеВДереве(ДеревоЗначения, "ВариантПоискаОбъекта", ЗначениеВариантаПоУмолчанию);
...
Функция СцТ_СформироватьОписаниеШага_НаКлиенте(Краткое = Ложь)
	...
	СДО = Новый Структура;
	СДО.Вставить("Вид"              , ЭлементФормы_Вид);
	СДО.Вставить("Имя"              , ЭлементФормы_Имя);
	СДО.Вставить("Заголовок"        , ЭлементФормы_Заголовок);
	СДО.Вставить("ПутьКЭлементу"    , ПутьКЭлементуФормы);
	
	ВозвращаемоеОписание = СцТ_ГлавнаяФорма.СцТ_ПолучитьОписаниеШага_НажатьКнопку(СДО, Краткое);
...
Процедура СцТ_ПолучитьИнтерактивныйКонтейнер()
	...
	Если ТипЗнч(ИнтерактивныйКонтейнер) <> Тип("ТестируемаяФорма")
		И ТипЗнч(ИнтерактивныйКонтейнер) <> Тип("ТестируемаяТаблицаФормы") Тогда
		ИнтерактивныйКонтейнер = Неопределено;
		Возврат;
	КонецЕсли;
...
Процедура ВыбратьКнопкуКИ(Команда)
	...
	Если ЭлементФормы_Вид = "Кнопка" Тогда
	...
	ИначеЕсли ЭлементФормы_Вид = "Декорация" Тогда
		
		ТипыВыбираемыхОбъектов.Добавить(Новый Структура("ТипОбъекта,ВидОбъекта",
	...
	ИначеЕсли ЭлементФормы_Вид = "Поле" Тогда
	...
	Иначе
		...
		ТипыВыбираемыхОбъектов.Добавить(ЭлементФормы_Вид);
