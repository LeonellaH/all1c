...
Процедура ПриОткрытии(Отказ)
	...
	ТекущийЭлемент = Элементы.ПаузаСекунды;
...
Процедура ПередЗакрытиеБезРежимаСовместимости(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если Модифицированность Тогда
		...
		Иначе
			ТекстПредупреждения = НСтр("ru = 'Данные шага ""Пауза"" были изменены. При закрытии все изменения будут утеряны'");
		КонецЕсли;
...
&НаКлиенте
Процедура ПаузаМинутыПриИзменении(Элемент)
...
Процедура СцТ_НастроитьФормуДляШага()
	...
	ВремяПаузыВСекундах = СцТ_ЗначениеВДереве(ДеревоЗначения, "ПродолжительностьПаузы", 0);
	
	ПаузаМинуты  = Цел(ВремяПаузыВСекундах/60);
	ПаузаСекунды = ВремяПаузыВСекундах - ПаузаМинуты*60;
...
Процедура СцТ_ПодготовитьДанныеШагаДляСохранения_НаКлиенте(Отказ)
	...
	ВремяПаузыВСекундах = ПаузаМинуты * 60 + ПаузаСекунды;
	
	СцТ_ГлавнаяФорма.СцТ_ДобавитьУзелВДеревоЗначения(
		ДеревоЗначения,
		"ПродолжительностьПаузы",
		ВремяПаузыВСекундах,
		НСтр("ru = 'Продолжительность паузы, сек.'"));
...
Функция СцТ_СформироватьОписаниеШага_НаКлиенте(Краткое = Ложь)
	...
	ВозвращаемоеОписание = НСтр("ru = 'Подождать'");
	
	Если ПаузаМинуты <> 0 Тогда
		ТекстНаименования = ВозвращаемоеОписание + " " + Строка(ПаузаМинуты) + НСтр("ru = ' мин.'");
	КонецЕсли;
	
	Возврат ВозвращаемоеОписание + " " + Строка(ПаузаСекунды) + НСтр("ru = ' сек.'");
...
&НаКлиенте
Процедура ПаузаСекундыПриИзменении(Элемент)
