...
Процедура ПриОткрытии(Отказ)
	...
	ТекущийЭлемент = Элементы.ОбластьПоискаСтроки;
...
Процедура ПередЗакрытиеБезРежимаСовместимости(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если Модифицированность Тогда
		...
		Иначе
			ТекстПредупреждения = НСтр("ru = 'Данные шага найти строку таблицы были изменены. При закрытии все изменения будут утеряны'");
		КонецЕсли;
...
&НаКлиенте
Процедура ЭлементыФормыТипЗначенияПриИзменении(Элемент)
	
	ТД = Элементы.ЭлементыФормы.ТекущиеДанные;
...
Процедура СцТ_НастроитьФормуДляШага()
	...
	СВОблП = Элементы.ОбластьПоискаСтроки.СписокВыбора;
	СВОблП.Добавить("СНачалаТаблицы"      , НСтр("ru = 'От начала таблицы вниз'"));
	СВОблП.Добавить("ОтТекущейСтроки"     , НСтр("ru = 'От текущей строки вниз'"));
	СВОблП.Добавить("ОтТекущейСтрокиВверх", НСтр("ru = 'От текущей строки вверх'"));
	СВОблП.Добавить("СКонцаТаблицы"       , НСтр("ru = 'От конца таблицы вверх'"));
	СВОблП.Добавить("ТекущаяСтрока"       , НСтр("ru = 'Только теущая строка'"));
	
	СВД = Элементы.НазначениеШагаПоискаСтроки.СписокВыбора;
	СВД.Добавить("СделатьТекущейСтрокой"    , НСтр("ru = 'Найти и сделать текущей строкой'"));
	СВД.Добавить("ПроверитьОтсутствиеСтроки", НСтр("ru = 'Проверить отсутствие строки'"));
	
	УзелКолонок = СцТ_НайтиУзелДереваФормы(ДеревоЗначения, "Имя", "КолонкиВСтроке");
	Если УзелКолонок <> Неопределено Тогда
		...
		Для каждого УзелКолонки Из УзелКолонок.ПолучитьЭлементы() Цикл
			...
			СтрЭлемента.Активность    = УзелКолонки.Активность;
			СтрЭлемента.Имя           = УзелКолонки.Имя;
			СтрЭлемента.Заголовок     = УзелКолонки.Синоним;
			СтрЭлемента.Значение      = УзелКолонки.Значение;
			СтрЭлемента.Представление = УзелКолонки.Представление;
			СтрЭлемента.Вид           = УзелКолонки.ТипУзла;
			...
			ИначеЕсли ТипЗнч(СтрЭлемента.Значение) = Тип("Строка") Тогда
				Если Лев(СтрЭлемента.Значение, 1) = "&"
					...
					СтрЭлемента.Значение = СтрЭлемента.Значение; // Сред(СтрЭлемента.Значение, 2); // Леонэлла
				Иначе
					СтрЭлемента.ТипЗначения = "Строка";
	...
	НазначениеШагаПоискаСтроки  = СцТ_ЗначениеВДереве(ДеревоЗначения, "НазначениеШага"           , "СделатьТекущейСтрокой");
	ОбластьПоискаСтроки        = СцТ_ЗначениеВДереве(ДеревоЗначения, "ОбластьПоиска"            , "СНачалаТаблицы");
	ИскатьВПодчиненныхСтроках = СцТ_ЗначениеВДереве(ДеревоЗначения, "ИскатьВПодчиненныхСтроках", Ложь);
...
Процедура СцТ_ПодготовитьДанныеШагаДляСохранения_НаКлиенте(Отказ)
	...
	Если ПустаяСтрока(НазначениеШагаПоискаСтроки) Тогда
		СцТ_СообщитьПользователю(НСтр("ru = 'Не указано назначение шага'"),
			Отказ,
			"НазначениеШагаНайтиФорму");
	КонецЕсли;
	
	Если ПустаяСтрока(ОбластьПоискаСтроки) Тогда
		СцТ_СообщитьПользователю(НСтр("ru = 'Не указана область поиска'"),
			Отказ,
			"ОбластьПоискаФормы");
	КонецЕсли;
	
	ВыбранКритерийПоиска = (ЭлементыФормы.НайтиСтроки(
		Новый Структура("Активность", Истина)).Количество() > 0);
	
	Если НЕ ВыбранКритерийПоиска Тогда
		СцТ_СообщитьПользователю(НСтр("ru = 'Не выбраны значения колонок для поиска строки'"),
			Отказ,
			"ТаблицаРеквизитов");
	КонецЕсли;
	...
	Если НЕ Отказ Тогда
		...
		УзелКолонок = СцТ_ГлавнаяФорма.СцТ_ДобавитьУзелВДеревоЗначения(ДеревоЗначения, "КолонкиВСтроке");
		УзлыКолонок = УзелКолонок.ПолучитьЭлементы();
		УзлыКолонок.Очистить();
		
		Для каждого СтрТаблРеквизитов Из ЭлементыФормы Цикл
			
			СцТ_ГлавнаяФорма.СцТ_ДобавитьУзелВДеревоЗначения(
				УзелКолонок,
				СтрТаблРеквизитов.Имя,
				СтрТаблРеквизитов.Значение,
				СтрТаблРеквизитов.Заголовок,
				СтрТаблРеквизитов.Активность,
				СтрТаблРеквизитов.Представление,
				,
				,
				СтрТаблРеквизитов.Вид);
...
Функция СцТ_СформироватьОписаниеШага_НаКлиенте(Краткое = Ложь)
	...
	Если Краткое Тогда
		Возврат ?(ОбластьПоискаСтроки = "ТекущаяСтрока",
				  НСтр("ru = 'Проверить текущую строку'"),
				  НСтр("ru = 'Найти строку таблицы'"));
	КонецЕсли;
	
	ВозвращаемоеОписание = "";
	Если ОбластьПоискаСтроки = "ТекущаяСтрока" Тогда
		
		ВозвращаемоеОписание = ?(НазначениеШагаПоискаСтроки = "СделатьТекущейСтрокой",
								 НСтр("ru = 'Проверить значения колонок текущей строки.'"),
								 НСтр("ru = 'Подтвердить отличие колонок текущей строки от заданных значений.'"));
		
	Иначе
		
		Если НазначениеШагаПоискаСтроки = "СделатьТекущейСтрокой" Тогда
			ВозвращаемоеОписание = НСтр("ru = 'Найти строку таблицы и сделать ее текущей.'");
		Иначе
			ВозвращаемоеОписание = НСтр("ru = 'Подтвердить отсутствие строки таблицы.'");
		КонецЕсли;
		
		ПредставлениеОбластиПоиска = "";
		
		Если ОбластьПоискаСтроки = "СНачалаТаблицы" Тогда
			ПредставлениеОбластиПоиска = НСтр("ru = 'от начала таблицы вниз.'");
			
		ИначеЕсли ОбластьПоискаСтроки = "ОтТекущейСтроки" Тогда
			ПредставлениеОбластиПоиска = НСтр("ru = 'от текущей строки вниз.'");
			
		ИначеЕсли ОбластьПоискаСтроки = "ОтТекущейСтрокиВверх" Тогда
			ПредставлениеОбластиПоиска = НСтр("ru = 'от текущей строки вверх.'");
			
		ИначеЕсли ОбластьПоискаСтроки = "СКонцаТаблицы" Тогда
			ПредставлениеОбластиПоиска = НСтр("ru = 'от конца таблицы вверх.'");
			
		КонецЕсли;
		
		ВозвращаемоеОписание = ВозвращаемоеОписание
			+ Символы.ПС
			+ НСтр("ru = 'Область поиска: '")
			+ ПредставлениеОбластиПоиска;
...
Процедура УстановитьТипЗначенияПоля(ТекущийТип, ДанныеСтроки)
	...
	Элементы.ЭлементыФормыЗначение.ОграничениеТипа = ОписаниеТипов;
	
	Элементы.ЭлементыФормыЗначение.ВыбиратьТип     = Ложь;
	Элементы.ЭлементыФормыЗначение.КнопкаВыбора    = Ложь;
