...
Процедура ПриОткрытии(Отказ)
	...
	ТекущийЭлемент = Элементы.ДействиеСТабДокументом_Действие;
...
Процедура ПередЗакрытиеБезРежимаСовместимости(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если Модифицированность Тогда
		...
		Иначе
			ТекстПредупреждения = НСтр("ru = 'Данные шага ""Действие с табличным документом"" были изменены. При закрытии все изменения будут утеряны'");
		КонецЕсли;
...
Процедура СцТ_НастроитьФормуДляШага()
	...
	Элементы.ДействиеСТабДокументом_Действие.СписокВыбора.Добавить("Проверить", "Проверить");
	Элементы.ДействиеСТабДокументом_Действие.СписокВыбора.Добавить("Заполнить", "Заполнить");
	
	ДействиеСТабДокументом_Действие = СцТ_ЗначениеВДереве(ДеревоЗначения, "ДействиеСТаблДок", "Проверить");
	
	ДействиеСТаблДок_ВариантОбласти = СцТ_ЗначениеВДереве(ДеревоЗначения, "ВариантОбласти"  , "ВесьТД");
	
	//УзелГраницОбласти = СцТ_НайтиУзелДереваФормы(ДеревоЗначения, "Имя", "ГраницыОбласти");
	
	ДействиеСТабДокументом_ПерваяКолонка     = СцТ_ЗначениеВДереве(ДеревоЗначения, "ПерваяКолонка"   , 0,, Истина);
	ДействиеСТабДокументом_ПоследняяКолонка  = СцТ_ЗначениеВДереве(ДеревоЗначения, "ПоследняяКолонка", 0,, Истина);
	ДействиеСТабДокументом_ПерваяСтрока      = СцТ_ЗначениеВДереве(ДеревоЗначения, "ПерваяСтрока"    , 0,, Истина);
	ДействиеСТабДокументом_ПоследняяСтрока   = СцТ_ЗначениеВДереве(ДеревоЗначения, "ПоследняяСтрока" , 0,, Истина);
	
	УправлениеДоступностьюНастроекОбластиТД(ЭтотОбъект);
	
	Если ДействиеСТаблДок_ВариантОбласти = "ЗаданнаяФормула" Тогда
		ДействиеСТабДокументом_Область = СцТ_ЗначениеВДереве(ДеревоЗначения, "ФормулаОбласти" , "",, Истина);
	КонецЕсли;
...
Процедура СцТ_ПодготовитьДанныеШагаДляСохранения_НаКлиенте(Отказ)
	...
	// Проверки
	Если ДействиеСТаблДок_ВариантОбласти = "ЗаданнаяОбласть" Тогда
		Если ДействиеСТабДокументом_ПерваяСтрока = 0 Тогда
			СцТ_СообщитьПользователю(НСтр("ru = 'Номер первой строки не может быть нулевым'"),
			Отказ,
			"ДействиеСТабДокументом_ПерваяСтрока");
		КонецЕсли;
		
		Если ДействиеСТабДокументом_ПерваяКолонка = 0 Тогда
			СцТ_СообщитьПользователю(НСтр("ru = 'Номер первой колонки не может быть нулевым'"),
			Отказ,
			"ДействиеСТабДокументом_ПерваяКолонка");
		КонецЕсли;
		
		Если ДействиеСТабДокументом_ПоследняяСтрока = 0 Тогда
			СцТ_СообщитьПользователю(НСтр("ru = 'Номер последней строки не может быть нулевым'"),
			Отказ,
			"ДействиеСТабДокументом_ПоследняяСтрока");
		КонецЕсли;
		
		Если ДействиеСТабДокументом_ПоследняяКолонка = 0 Тогда
			СцТ_СообщитьПользователю(НСтр("ru = 'Номер последней колонки не может быть нулевым'"),
			Отказ,
			"ДействиеСТабДокументом_ПоследняяКолонка");
		КонецЕсли;
		
		Если ДействиеСТабДокументом_ПерваяСтрока > ДействиеСТабДокументом_ПоследняяСтрока Тогда
			СцТ_СообщитьПользователю(НСтр("ru = 'Номер первой строки не может быть больше, чем номер последней'"),
				Отказ,
				"ДействиеСТабДокументом_ПоследняяСтрока");
		КонецЕсли;
		
		Если ДействиеСТабДокументом_ПерваяКолонка > ДействиеСТабДокументом_ПоследняяКолонка Тогда
			СцТ_СообщитьПользователю(НСтр("ru = 'Номер первой колонки не может быть больше, чем номер последней'"),
				Отказ,
				"ДействиеСТабДокументом_ПоследняяКолонка");
		КонецЕсли;
		
	ИначеЕсли ДействиеСТаблДок_ВариантОбласти = "ЗаданнаяФормула" Тогда
		Если ПустаяСтрока(ДействиеСТабДокументом_Область)  Тогда
			СцТ_СообщитьПользователю(НСтр("ru = 'Область должна быть задана'"),
			Отказ,
			"ДействиеСТабДокументом_Область");
		КонецЕсли;
		
		Если Лев(ДействиеСТабДокументом_Область, 1) = "&" Тогда
			// Возможно, что это параметр
			ТекстСообщения = "";
			Если НЕ СцТ_ГлавнаяФорма.ПроверитьПравильностьУказателяНаПараметр(ДействиеСТабДокументом_Область, Истина, ТекстСообщения) Тогда
		...
		Иначе
			Попытка
				КоординатыОбласти = СцТ_ГлавнаяФорма.СцТ_КоординатыОбластиИзАдреса(ДействиеСТабДокументом_Область);
			Исключение
				СцТ_СообщитьПользователю(НСтр("ru = 'Не верно задана область'"),
					Отказ,
					"ДействиеСТабДокументом_Область");
			КонецПопытки;
	...
	СцТ_ГлавнаяФорма.СцТ_ДобавитьУзелВДеревоЗначения(
		ДеревоЗначения,
		"ДействиеСТаблДок",
		ДействиеСТабДокументом_Действие);
		
	СцТ_ГлавнаяФорма.СцТ_ДобавитьУзелВДеревоЗначения(
		ДеревоЗначения,
		"ВариантОбласти",
		ДействиеСТаблДок_ВариантОбласти);
	
	УзелОбласти = СцТ_ГлавнаяФорма.СцТ_ДобавитьУзелВДеревоЗначения(
		ДеревоЗначения,
		"ГраницыОбласти");
...
Функция СцТ_СформироватьОписаниеШага_НаКлиенте(Краткое = Ложь)
	...
	Если ДействиеСТабДокументом_Действие = "Проверить" Тогда
		ВозвращаемоеОписание = НСтр("ru = 'Сверить с эталонными значениями'");
	Иначе
		ВозвращаемоеОписание = НСтр("ru = 'Заполнить'");
	КонецЕсли;
	
	Если ДействиеСТаблДок_ВариантОбласти = "ВесьТД" Тогда
		ВозвращаемоеОписание = СцТ_ГлавнаяФорма.СцТ_ДобавитьПодстроку(ВозвращаемоеОписание, НСтр("ru = 'весь хранимый табличный документ'"), " ");
	Иначе
		ВозвращаемоеОписание = СцТ_ГлавнаяФорма.СцТ_ДобавитьПодстроку(ВозвращаемоеОписание, НСтр("ru = 'заданную область табличного документа'"), " ");
	КонецЕсли;
...
Процедура ОбновитьПоДаннымФормы(Команда)
	...
	Если ДействиеСТабДокументом_Действие = "ПроверитьЗаголовок" Тогда
		Попытка
	...
	ИначеЕсли ДействиеСТабДокументом_Действие = "ПроверитьМодифицированность" Тогда
		Попытка
...
&НаКлиенте
Процедура ДействиеСТаблДок_ВариантОбластиПриИзменении(Элемент)
...
&НаКлиенте
Процедура ДействиеСТабДокументом_ПерваяСтрокаПриИзменении(Элемент)
	
	СформироватьФормулуОбласти(ЭтотОбъект);
	СцТ_ОбновитьАвтоОписаниеШага_НаКлиенте(Истина);
	
КонецПроцедуры

&НаКлиенте
Процедура ДействиеСТабДокументом_ПерваяКолонкаПриИзменении(Элемент)
...
&НаКлиенте
Процедура ДействиеСТабДокументом_ПоследняяСтрокаПриИзменении(Элемент)
...
&НаКлиенте
Процедура ДействиеСТабДокументом_ПоследняяКолонкаПриИзменении(Элемент)
...
&НаКлиенте
Процедура ДействиеСТабДокументом_ОбластьПриИзменении(Элемент)
	
	Если НЕ СцТ_ГлавнаяФорма.ПроверитьПравильностьУказателяНаПараметр(ДействиеСТабДокументом_Область) Тогда
...
&НаКлиентеНаСервереБезКонтекста
Процедура УправлениеДоступностьюНастроекОбластиТД(УпрФорма)
	
	Если УпрФорма.ДействиеСТаблДок_ВариантОбласти = "ВесьТД" Тогда
		
		УпрФорма.ДействиеСТабДокументом_ПерваяКолонка    = 0;
		УпрФорма.ДействиеСТабДокументом_ПоследняяКолонка = 0;
		УпрФорма.ДействиеСТабДокументом_ПерваяСтрока     = 0;
		УпрФорма.ДействиеСТабДокументом_ПоследняяСтрока  = 0;
		
		УпрФорма.Элементы.ДействиеСТабДокументом_ПерваяКолонка.Доступность    = Ложь;
		УпрФорма.Элементы.ДействиеСТабДокументом_ПоследняяКолонка.Доступность = Ложь;
		УпрФорма.Элементы.ДействиеСТабДокументом_ПерваяСтрока.Доступность     = Ложь;
		УпрФорма.Элементы.ДействиеСТабДокументом_ПоследняяСтрока.Доступность  = Ложь;
		
		УпрФорма.Элементы.ДействиеСТабДокументом_ПерваяКолонка.МинимальноеЗначение    = 0;
		УпрФорма.Элементы.ДействиеСТабДокументом_ПоследняяКолонка.МинимальноеЗначение = 0;
		УпрФорма.Элементы.ДействиеСТабДокументом_ПерваяСтрока.МинимальноеЗначение     = 0;
		УпрФорма.Элементы.ДействиеСТабДокументом_ПоследняяСтрока.МинимальноеЗначение  = 0;
		
		УпрФорма.Элементы.ДействиеСТабДокументом_Область.Доступность          = Ложь;
		
	ИначеЕсли УпрФорма.ДействиеСТаблДок_ВариантОбласти = "ЗаданнаяОбласть" Тогда
		УпрФорма.Элементы.ДействиеСТабДокументом_ПерваяКолонка.Доступность    = Истина;
		УпрФорма.Элементы.ДействиеСТабДокументом_ПоследняяКолонка.Доступность = Истина;
		УпрФорма.Элементы.ДействиеСТабДокументом_ПерваяСтрока.Доступность     = Истина;
		УпрФорма.Элементы.ДействиеСТабДокументом_ПоследняяСтрока.Доступность  = Истина;
		
		УпрФорма.Элементы.ДействиеСТабДокументом_Область.Доступность          = Ложь;
		
		Если УпрФорма.ДействиеСТабДокументом_ПерваяКолонка    = 0 Тогда
			УпрФорма.ДействиеСТабДокументом_ПерваяКолонка    = 1;
		КонецЕсли;
		
		Если УпрФорма.ДействиеСТабДокументом_ПоследняяКолонка = 0 Тогда
			УпрФорма.ДействиеСТабДокументом_ПоследняяКолонка    = 1;
		КонецЕсли;
		
		Если УпрФорма.ДействиеСТабДокументом_ПерваяСтрока     = 0 Тогда
			УпрФорма.ДействиеСТабДокументом_ПерваяСтрока    = 1;
		КонецЕсли;
		
		Если УпрФорма.ДействиеСТабДокументом_ПоследняяСтрока  = 0 Тогда
			УпрФорма.ДействиеСТабДокументом_ПоследняяСтрока    = 1;
		КонецЕсли;
		
		УпрФорма.Элементы.ДействиеСТабДокументом_ПерваяКолонка.МинимальноеЗначение    = 1;
		УпрФорма.Элементы.ДействиеСТабДокументом_ПоследняяКолонка.МинимальноеЗначение = 1;
		УпрФорма.Элементы.ДействиеСТабДокументом_ПерваяСтрока.МинимальноеЗначение     = 1;
		УпрФорма.Элементы.ДействиеСТабДокументом_ПоследняяСтрока.МинимальноеЗначение  = 1;
		
		СформироватьФормулуОбласти(УпрФорма);
	...
	Иначе
		...
		УпрФорма.Элементы.ДействиеСТабДокументом_Область.Доступность          = Истина;
...
&НаКлиентеНаСервереБезКонтекста
Процедура СформироватьФормулуОбласти(УпрФорма)
	
	Если УпрФорма.ДействиеСТабДокументом_ПерваяКолонка = УпрФорма.ДействиеСТабДокументом_ПоследняяКолонка
		И УпрФорма.ДействиеСТабДокументом_ПерваяСтрока = УпрФорма.ДействиеСТабДокументом_ПоследняяСтрока Тогда
		// Это ячейка
		ИмяОбласти = "R%1C%2";
	Иначе
		// Это Область
		ИмяОбласти = "R%1C%2:R%3C%4";
	КонецЕсли;
	
	ИмяОбласти = СтрЗаменить(ИмяОбласти, "%1", Формат(УпрФорма.ДействиеСТабДокументом_ПерваяСтрока    , "ЧГ=0; ЧН=0"));
	ИмяОбласти = СтрЗаменить(ИмяОбласти, "%2", Формат(УпрФорма.ДействиеСТабДокументом_ПерваяКолонка   , "ЧГ=0; ЧН=0"));
	ИмяОбласти = СтрЗаменить(ИмяОбласти, "%3", Формат(УпрФорма.ДействиеСТабДокументом_ПоследняяСтрока , "ЧГ=0; ЧН=0"));
	ИмяОбласти = СтрЗаменить(ИмяОбласти, "%4", Формат(УпрФорма.ДействиеСТабДокументом_ПоследняяКолонка, "ЧГ=0; ЧН=0"));
	
	УпрФорма.ДействиеСТабДокументом_Область = ИмяОбласти;
...
Процедура СцТ_ОбновитьАвтоОписаниеШага_НаКлиенте(ОбновитьНаименование = Ложь)
	...
	Автоописание = СцТ_СформироватьОписаниеШага_НаКлиенте();
		...
		Если НЕ ДШ_СкрытьАвтоописание И НЕ ПустаяСтрока(АвтоОписание) Тогда
